...
public class ValidatorTest {
	private final static String MANIFEST_FILE = "r2rml/validator-test-manifest.ttl";
	private final static String SCHEMA = "r2rml/schemas/validator-test.sql";
		...
		for (Message actual: report.getMessages()) {
			...
			if (!match) {
				if (actual.getCause() != null) {
					LoggerFactory.getLogger(ValidatorTest.class).error(
							"Following unexpected message has associated Throwable", 
							actual.getCause());
				}
